...
import { BookOpen, FileText, Video, Gamepad2, ExternalLink, Play, Download } from 'lucide-react';
          ...
          {materials.map((mat) => (
            ...
            >
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <span className="text-[10px] uppercase font-black bg-sky-50 text-sky-800 px-2.5 py-0.5 rounded-md border border-sky-100">
                    {mat.type === 'game_iframe' ? 'GAME TRỰC TUYẾN' : mat.type}
                  </span>
              ...
              <div className="pt-2 flex justify-end">
                {mat.type === 'game_iframe' ? (
                  <button
                    onClick={() => setActiveIframeUrl(mat.file_url)}
                    className="bg-amber-500 hover:bg-amber-600 text-white font-extrabold px-5 py-2.5 rounded-2xl shadow-md transition-all text-xs flex items-center space-x-1.5 active:scale-95 cursor-pointer"
                  >
                    <Play className="w-4 h-4 fill-current" />
                    <span>MỞ TRẢI NGHIỆM GAME</span>
                  </button>
                ) : (
                  <a
                    href={mat.file_url}
                    target="_blank"
                    rel="noreferrer"
                    className="bg-sky-600 hover:bg-sky-700 text-white font-extrabold px-5 py-2.5 rounded-2xl shadow-md transition-all text-xs flex items-center space-x-1.5 active:scale-95"
                  >
                    <ExternalLink className="w-4 h-4" />
                    <span>XEM HỌC LIỆU / TẢI VỀ</span>
                  </a>
                )}
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex flex-col items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-4xl w-full h-[85vh] flex flex-col overflow-hidden shadow-2xl">
            <div className="p-4 bg-slate-900 text-white flex justify-between items-center">
              <span className="font-bold text-sm">🎮 Trải Nghiệm Học Liệu / Game Giáo Dục</span>
              <button
                onClick={() => setActiveIframeUrl(null)}
                className="bg-rose-500 hover:bg-rose-600 text-white font-black px-4 py-1.5 rounded-xl text-xs"
              >
                Đóng Player
              </button>
            </div>
